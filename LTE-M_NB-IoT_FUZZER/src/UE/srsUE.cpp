...
bool SRSUE::start()
{
    ...
    if (main_config_g.use_monitor) {
        ...
        "/srsue/src/srsue --gw.netns=ue1" + (srsran_config_g.close_ue_after_fuzzing ? "" : ";bash") + "\"";
    } else {
        ...
        "/srsue/src/srsue --gw.netns=ue1 > /dev/null 2>&1 &";
    }
...
const std::string& SRSUE::get_name() const
{
    return name_;
}
...
bool SRSUE_Base::stop()
{
    ...
    int signal = SIGKILL;
    std::string command = "sudo killall -s " + std::to_string(signal) + " srsue";
...
bool SRSUE_Base::is_alive()
{
    if (system("sudo killall -s 0 srsue > /dev/null 2>&1") != 0) {
        return false;
    }
    return true;
}